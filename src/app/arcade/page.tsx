...
export default function ArcadePage() {
  ...
  const connectCallbacks = useCallback((): Parameters<typeof connect>[1] => ({
    onSync(players) {
      ...
      // Snap camera to local player on first sync
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // 1. Load map + fetch avatar in parallel
      const [map, avatarRes] = await Promise.all([
        loadMap("/maps/lobby.json"),
      ...
      if (avatarRes.config?.sprite_id !== undefined) {
        ...
        connect(token, connectCallbacks(), avatarRes.config.sprite_id);
      } else {
  ...
  }, [router, isTyping, connectCallbacks]);
  ...
  const handleAvatarConfirm = async () => {
    ...
    try {
      ...
      if (isFirstTime) {
        ...
        connect(tokenRef.current, connectCallbacks(), selectedSprite);
      } else {
